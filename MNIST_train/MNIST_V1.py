...
#     # 定义多项式特征扩展（选择适当的degree和参数以控制特征维度）
#     # degree=2 会产生 ~307,760 特征，这里为了控制特征维度，我们选择degree=2并使用sparse=True
...
#     # 保存模型（可选）
#     joblib.dump(pipeline, 'svm_mnist_poly2.joblib')
...
def main():
    ...
    # 保存模型（可选）
    joblib.dump(pipeline, 'sgd_mnist_poly2.joblib')
