...
# # 模型评估
# def dev_eval(model, data, loss_function, device):
#     '''
#     :param model: 模型
...
#         dev_acc, dev_loss, _, _ = dev_eval(model, dataloaders['dev'], loss_function, device)
#         scheduler.step(dev_loss)
#         if dev_loss < dev_best_loss:
#             dev_best_loss = dev_loss
#             if isinstance(model, nn.DataParallel):
#                 torch.save(model.module.state_dict(), current_save_path)
#             else:
...
#         print(f"Epoch {i+1}/{num_epochs} - train_loss: {train_loss:.3f}, train_acc: {train_acc:.2f}, "
#               f"dev_loss: {dev_loss:.3f}, dev_acc: {dev_acc:.2f}")
...
#     if isinstance(model, nn.DataParallel):
#         model.module.load_state_dict(torch.load(current_save_path))
#     else:
#         model.load_state_dict(torch.load(current_save_path))
#     model.eval()
#     start_time_eval = time.time()
#     test_acc, test_loss, test_labels, test_probs = dev_eval(model, dataloaders['test'], loss_function, device)
...
#     print(f"Evaluation Time: {get_time_dif(start_time_eval)}")
...
#     dataloaders = {
#         'train': DataLoader(TextDataset(train_data), batch_size, shuffle=True, num_workers=4, pin_memory=True),
#         'dev': DataLoader(TextDataset(dev_data), batch_size, shuffle=False, num_workers=4, pin_memory=True),
#         'test': DataLoader(TextDataset(test_data), batch_size, shuffle=False, num_workers=4, pin_memory=True)
#     }
...
def train(model, dataloaders, optimizer_type='adam'):
    ...
    # **关键步骤：调用 flatten_parameters()**
    if hasattr(model, 'module') and hasattr(model.module, 'lstm'):
        model.module.lstm.flatten_parameters()
    ...
    for i in range(num_epochs):
        ...
        # 每个epoch后调用 flatten_parameters() 以确保权重连续性
        if hasattr(model, 'module') and hasattr(model.module, 'lstm'):
            model.module.lstm.flatten_parameters()
        elif hasattr(model, 'lstm'):
            model.lstm.flatten_parameters()
